...
public class JsonToClassRequest<T> extends Request<T> {
  ...
  @Override public byte[] getBody() throws AuthFailureError {
    if (!TextUtils.isEmpty(mRequestBody)) {//如果是字符串型的post参数
      try {
  ...
  @Override public String getCacheKey() {
    if (TextUtils.isEmpty(mCacheKey)) {//如果没有额外设置缓存key
      String postParam = mRequestBody;//首先取字符串形式的post参数
      if (TextUtils.isEmpty(postParam)) {//如果没有获取到字符串参数，就获取Map类型的Post参数
        byte[] body = null;
        try {
          body = getBody();
