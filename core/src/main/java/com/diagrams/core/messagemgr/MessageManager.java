...
public final class MessageManager {
    ...
    public abstract static class Caller<T extends IObserverBase> implements Runnable {
        ...
        public void run() {
            if (!silence) {
                ...
                ProcessingNotifyStack.ProcessingItem processingItem =
                        ProcessingNotifyStack.push(processNotifyID, list.size());
                while (processingItem.pos < processingItem.total) {
                    @SuppressWarnings("unchecked") T item = (T) list.get(processingItem.pos);
                    ob = item;
                    call();
                    ++processingItem.pos;
                }
    ...
    public void detachMessage(final MessageID id, final IObserverBase ob) {
        ...
        KwDebug.classicAssert(false,
                "没有attach就要detach或者detach多次" + ", id = [" + id + "], ob = [" + ob + "]");
    }
    ...
    // 在某个handler里同步执行
    public <T extends IObserverBase> void syncRunTargetHandler(final Handler handler,
            final Caller<T> r) {
        ...
        if (handler.getLooper().getThread().getId() == Thread.currentThread().getId()) {
            ...
            try {
                synchronized (r) {
                    ...
                    if (handler == mainThreadMsgHandler && AppSupplier.isExiting()) {
        ...
        if (t > 150 && Thread.currentThread().getId() == AppSupplier.getMainThreadID()) {
    ...
    // 在某个handler里异步执行
    public <T extends IObserverBase> void asyncRunTargetHandler(final Handler handler,
            final int delayMiliseconds, final Caller<T> r) {
        if (AppSupplier.IS_DEBUG) {
            final String stackInfoString = KwDebug.createStackInfo();
            handler.postDelayed(new Runnable() {
                ...
                public void run() {
                    ...
                    long t = System.currentTimeMillis() - startTime;
                    if (t > 150
                            && Thread.currentThread().getId() == AppSupplier.getMainThreadID()) {
                        LogMgr.w(TAG, stackInfoString);
                        LogMgr.w(TAG, "消息执行超时，time=" + t);
    ...
    static final long mainThreadId = AppSupplier.getMainThreadID();
    static final Handler mainThreadMsgHandler = AppSupplier.getMainThreadHandler();
